...
function localBusiness(ctx: PageSchemaContext) {
  ...
  return {
    ...
    "@id": entityId,
    name: (brand as { dbaName?: string }).dbaName || brand.displayName,
    alternateName: [brand.displayName, brand.legalName].filter(
      (n, i, a) => n !== ((brand as { dbaName?: string }).dbaName || brand.displayName) && a.indexOf(n) === i),
...
function organization() {
  return {
    ...
    name: (brand as { dbaName?: string }).dbaName || brand.legalName,
    alternateName: brand.shortName,
    url: brand.canonicalUrl,
